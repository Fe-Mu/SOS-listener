...
from kivymd.uix.button import MDRectangleFlatButton, MDRaisedButton, MDFillRoundFlatIconButton, MDFillRoundFlatButton
...
class MainApp(MDApp):
    ...
    def build(self):
        ...
        donations_link = self.store2.get("donations_link")["link"]
        contact_email = self.store2.get("contact_email")["email"]
        version = self.store2.get("version")["version"]
        ...
        self.layout_version = MDBoxLayout(size_hint=(1, .05))
        ...
        self.version_btn = MDRectangleFlatButton(text='Version ' + version
                                                 , size_hint=(1, 1)
                                                 , font_size='12dp'
                                                 # , line_color='lightgrey'
                                                 , line_color='white'
        ...
        self.layout_version.add_widget(self.version_btn)
        self.layout_main_app.add_widget(self.layout_version)
...
"""
        ...
        s9_donate_btn = MDFillRoundFlatIconButton(text='Donate'
                                                  , size_hint_x=None
                                                  , width='50dp'
                                                  , height='30dp'
                                                  , pos_hint={'center_x': 0.5, 'center_y': 0.5}
        ...
        s9_donate_btn.bind(on_release=partial(webbrowser.open, donations_link))

        s9_support_lbl = Label(
        ...
        s9_support_lbl.bind(texture_size=lambda instance, value: setattr(instance, 'height', value[1]))
        s9_support_lbl.bind(width=lambda instance, value: setattr(instance, 'text_size', (value, None)))
        ...
        s9_layout.add_widget(s9_donate_btn)
        s9_layout.add_widget(s9_support_lbl)
...
List of known packages or licenses used in this project.
...
"""
        ...
        # Test mode feature
        if self.store2.get("test_mode")["selected"] == 'no':
            self.status_test_mode = False
            self.store2.put("test_mode", selected='no')

        if self.store2.get("test_mode")["selected"] == 'yes':
            self.status_test_mode = True
            self.store2.put("test_mode", selected='yes')
        ...
        if self.store2.get("light_theme")["selected"] == 'no':
            ...
            self.version_btn.line_color = 'grey'
            self.version_btn.md_bg_color = (.12, .12, .12, 1)
            self.version_btn.text_color = (.02, .91, 1, 1)
        ...
        if self.store2.get("light_theme")["selected"] == 'yes':
            ...
            self.version_btn.line_color = (.85, .85, .85, 1)
            self.version_btn.md_bg_color = 'white'
            self.version_btn.text_color = 'grey'
    ...
    def on_gps_location(self, **kwargs):
        ...
        # Coordinates for the "on_gps_location" tutorial
        #kwargs['lat'] = "1.11"
        #kwargs['lon'] = "1.11"
    ...
    def show_popup(self, instance):
        ...
        layout_top0 = MDBoxLayout(orientation='horizontal')
        ...
        # Layouts colorwheel
        self.layout_colorpicker = MDBoxLayout(orientation='vertical'
                                              , size_hint=(5, 5)
                                              , padding='20dp'
                                              , spacing='15dp')
        ...
        self.testmode_lbl = Label(text='Test Mode'
                                  , color='lightgrey'
                                  , pos_hint={'center_x': 0.5, 'center_y': 0.5}
                                  , size_hint=(1, 1)
                                  )
        self.testmode_empty_lbl = Label(text=''
                                        , color='lightgrey'
                                        , pos_hint={'center_x': 0.5, 'center_y': 0.5}
                                        , size_hint=(0.5, None)
                                        , height='40dp'
                                        )
        self.testmode_switch = Switch(active=self.status_test_mode
                                      , size_hint=(0.5, 1)
                                      )
        self.testmode_switch.bind(active=self.testmode_on)
        ...
        self.colorpicker = ColorWheel(size_hint=(1, 1)
                                      , pos_hint={'center_x': 0.5, 'center_y': 0.5}
                                      )
        self.colorpicker.bind(color=self.on_color)

        self.label_color = Button(text='Choose color'
                                  , font_size='13dp'
                                  , color='black'
        ...
        layout_top0.add_widget(self.testmode_switch)
        layout_top0.add_widget(self.testmode_lbl)
        layout_top0.add_widget(self.testmode_empty_lbl)
        ...
        self.layout_colorpicker.add_widget(self.label_color)
        self.layout_colorpicker.add_widget(self.colorpicker)

        # Adds secondary layouts to main show_popup layout
        layout_show_popup_main.add_widget(layout_top0)
        ...
        layout_show_popup_main.add_widget(self.layout_colorpicker)
    ...
    def choose_popup(self, instance):
        ...
        btn_test_sos = MDRectangleFlatButton(on_press=self.test_sos
                                             , text="Test \nSOS"
                                             , halign='center'
                                             , font_size='17dp'
                                             , size_hint=(0.6, 0.2)
                                             ...
                                             , line_color=(.5, 1, 1, 1)
                                             , text_color='lightgrey'
                                             , disabled=not self.status_test_mode
                                             )

        # Add widgets to popup
        if self.status_test_mode == False:
        ...
        if self.status_test_mode == True:
            layout_choose_popup.add_widget(btn_call)
            layout_choose_popup.add_widget(btn_test_sos)
            layout_choose_popup.add_widget(btn_cancel)
    ...
    def save_data(self, instance):
        ...
        color_save = self.colorpicker.color
        ...
        if save_name not in self.store and save_name is not "":
            ...
            self.store.put(save_name, country=country, telephone=telephone, color=color_save)

            # Creates a new button for the emergence-contact screen
            btn_add_nex_contact = Button(text=save_name
                                         , font_size='20dp'
                                         , color=(0, 0, 0, 1)
                                         , background_color=color_save
                                         , background_normal=''
                                         , size_hint=(1, None)
                                         , height='150dp'
                                         )
            ...
            btn_add_nex_contact.background_color = color_save
    ...
    def update_data(self, instance):
        ...
        if self.store.exists(update_name):
            ...
            if self.colorpicker.color == [0, 0, 0, 0]:
            #if self.store.get(update_name)["color"] is not self.colorpicker.color and self.colorpicker.color is [0,0,0,0]:
                color_update = self.store.get(update_name)["color"]
            else:
                color_update = self.colorpicker.color

            # Updates existing contact with new information
            self.store.put(update_name, country=country, telephone=telephone, color=color_update)
            ...
            for existing_btn in self.layout_contacts.children:
                if existing_btn.text == update_name:
                    ...
                    existing_btn.background_color = color_update
                    break
    ...
    def send_sos(self, instance):
        ...
        if self.store.exists(name):
            ...
            sms.send(recipient="+" + self.store[name]['country'] + self.store[name]['telephone']
                     ,
                     message=f"Location:\nhttps://comaps.at/map?v=1&ll={self.store2.get('lat')['latitude']},{self.store2.get('lon')['longitude']}"
                     )
    ...
    # SMS Test mode feature
    def testmode_on(self, instance, value):
    ...
    # Changes selected color on colorwheel
    def on_color(self, instance, *args):
        self.label_color.background_color = instance.color
    ...
    def backup_contacts(self, *args):
        ...
        # Added by L.F.G Muñoz
        backup_pass = Label(
            text="A backup of all emergency-contacts has been saved in the /Documents/SOS-listener directory."
            , size_hint_y=None
            , valign='top'
            , halign='left'
            , markup=True
        ...
        try:
            ...
            filename = os.path.join(SharedStorage().get_cache_dir(),
                                    str(today.strftime(
                                        "%Y.%m.%d")) + "_sosListener_backup.txt")  # forming the path of our new file
